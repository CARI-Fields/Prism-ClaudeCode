...
def test_execute_runs_launcher_collects_both_and_writes_meta(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(R, "collect", lambda *a, **k: [tmp_path / "sess.jsonl"])
    monkeypatch.setattr(R, "collect_tap", lambda rd, since, until: [tmp_path / "abc.json"])
    ...
    assert meta["transcripts"] == ["sess.jsonl"]
    assert meta["tap"] == ["abc.json"]
