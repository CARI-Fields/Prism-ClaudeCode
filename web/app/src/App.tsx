...
const Section1View = lazy(() => import('./views/Section1View').then((m) => ({ default: m.Section1View })));
const Section2View = lazy(() => import('./views/Section2View').then((m) => ({ default: m.Section2View })));
const Section3View = lazy(() => import('./views/Section3View').then((m) => ({ default: m.Section3View })));
  ...
  return (
    <AppStateProvider manifest={data.manifest}>
      <AppShell manifest={data.manifest} sidebar={<FilterRail />}
        canvas={<><ViewNav /><Suspense fallback={<ViewSkeleton />}><ViewCanvas views={views} /></Suspense></>} />
    </AppStateProvider>
  );
}
function Gate() {
