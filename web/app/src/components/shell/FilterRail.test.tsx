...
const manifest: Manifest = { variants: [{ key: 'r1', eyebrow: '', title: 'R1', lede: '', conditions: ['goal', 'subagents'], tasks: ['coding_longhorizon', 'research_longhorizon'] }],
  strategy_desc: {}, task_meta: {}, available: [] };
vi.mock('../../data/DataContext', () => ({ useData: () => ({ data: { manifest, runs: [{ run_id: 'a', task: 'coding_longhorizon', condition: 'goal', rep: 1 }], turns: [] } }) }));
function Probe() { const f = useFilter(); return <span>cond:{f.filter.condition.join(',')}|task:{f.filter.task.join(',')}</span>; }
