...
export function FilterRail() {
  ...
  const variantRuns = useMemo(
    () =>
      (data?.runs ?? []).filter(
        (r) =>
          variant && variant.tasks.includes(r.task) && variant.conditions.includes(r.condition),
      ),
    [data, variant],
  );
  const reps = useMemo(
  ...
  const variantTurns = useMemo(
    () =>
      (data?.turns ?? []).filter(
        (t) =>
          variant && variant.tasks.includes(t.task) && variant.conditions.includes(t.condition),
      ),
    [data, variant],
  );
  ...
  return (
    <div className="rail">
      <div className="rail-top">
        <span className="rail-title">Filters</span>
        {anyActive ? (
          <Button
            minimal
            small
      ...
      <RailFilterGroup
        label="Feature"
        items={variant.conditions}
        active={filter.condition}
        dotFor={conditionColor}
        onToggle={(t) => toggle('condition', t)}
