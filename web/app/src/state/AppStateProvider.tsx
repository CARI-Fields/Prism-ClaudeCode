...
function bootstrap(manifest: Manifest): UiState {
  ...
  const report =
    url.report && manifest.variants.some((v) => v.key === url.report)
      ? url.report
      : (manifest.variants[0]?.key ?? '');
...
export function AppStateProvider({
  ...
  useEffect(() => {
    const next = toHash({
      ...
      view: state.view,
      filter: state.filter,
      s3Condition: state.overrides.s3?.condition ?? [],
    });
  ...
  return (
    <StateCtx.Provider value={value}>
      <div className={`app-root${state.theme === 'dark' ? ' bp5-dark' : ''}`}>{children}</div>
    </StateCtx.Provider>
  );
}
...
export function useTheme() {
  ...
  return {
    mode: state.theme,
    toggle: () => setState((s) => R.setTheme(s, s.theme === 'dark' ? 'light' : 'dark')),
...
export function useFilter() {
  ...
  return {
    ...
    setOverrideSingle: (view: ViewKey, dim: FilterDim, t: string) =>
      setState((s) => R.setOverrideSingle(s, view, dim, t)),
