...
export interface MatrixCell {
  ...
  row: string;
  status: Status;
  status_code: (typeof CODE)[Status];
  run_id?: string;
  ...
  for (const task of tasks)
    for (const rep of reps)
      for (const condition of conditions) {
        const match = runs.filter(
          (r) => r.task === task && r.condition === condition && r.rep === rep,
        );
        ...
        const skipped =
          String((latest as { status?: unknown }).status ?? '').toLowerCase() === 'skipped';
