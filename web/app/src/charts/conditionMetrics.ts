...
export interface MetricRow {
  ...
  mean_num_requests: number | null;
  mean_cache_hit_ratio: number | null;
  mean_total_cache_read: number | null;
  mean_peak_prompt_tokens: number | null;
  mean_output_tokens_total: number | null;
  mean_total_cost_usd: number | null;
  mean_quality_score: number | null;
  mean_cost_efficiency_score: number | null;
...
const COLUMN_OF: Record<MeanKey, string> = {
  mean_completion_time_s: 'completion_time_s',
  mean_num_requests: 'num_requests',
  mean_cache_hit_ratio: 'cache_hit_ratio',
  mean_total_cache_read: 'total_cache_read',
  mean_peak_prompt_tokens: 'peak_prompt_tokens',
  mean_output_tokens_total: 'output_tokens_total',
  mean_total_cost_usd: 'total_cost_usd',
  mean_quality_score: 'quality_score',
  mean_cost_efficiency_score: 'cost_efficiency_score',
...
function metricRow(task: string, condition: string, runs: Run[]): MetricRow {
  const row = {
    ...
    condition,
    runs: runs.length,
    success_rate: mean(runs, 'success'),
  } as MetricRow;
    ...
    for (const condition of conditions) {
      out.push(
        metricRow(
          task,
          condition,
          taskRuns.filter((r) => r.condition === condition),
        ),
      );
    }
  }
...
export interface OverheadRow {
  task: string;
  condition: string;
  num_requests_factor: number | null;
  completion_time_factor: number | null;
  total_cost_factor: number | null;
  peak_prompt_tokens_factor: number | null;
  total_cache_read_factor: number | null;
  output_tokens_factor: number | null;
}
...
export function conditionOverheads(
  metrics: MetricRow[],
  tasks: string[],
  conditions: string[],
): OverheadRow[] {
