...
def score_research_report(
    ...
    if not path.exists():
        return _empty_score(False, list(required_sections))
    return score_research_text(path.read_text(), required_sections)
...
def score_research_text(
    ...
    for section in required:
        ...
        if len(_unique_urls(body)) == 2:
            exact_two_url_sections += 1
        coverage_scores.append(_coverage_score(section, body) if body else 0.0)
    ...
    word_count_score = _word_count_score(word_count)
    urls = _unique_urls(text)
    unique_url_score = min(1.0, len(urls) / 12)
    format_score = (
        0.35 * section_score
...
def _coverage_score(section: str, text: str) -> float:
    groups = SECTION_KEYWORD_GROUPS.get(section, ())
    if not groups:
        return 0.0
    lower = (text or "").lower()
...
def _word_count_score(word_count: int) -> float:
    if 900 <= word_count <= 1400:
        return 1.0
    if word_count < 900:
        return max(0.0, word_count / 900)
    return max(0.0, 1.0 - ((word_count - 1400) / 700))
